// dashboard_custom.js
// Versão 30 - Ajuste definitivo para submissão de formulários SPA.
console.info("dashboard_custom.js (Versão 30) carregado. Configurando navegação SPA.");
...
    'anuncio/index': 'anuncio.js', // anexo aqui para garantir que carregue quando precisar
...
async function handleSpaFormSubmit(event) {
    ...
    try {
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            // Se o JSON contiver HTML, usa-o. Caso contrário, assume-se que foi uma ação de sucesso sem alteração de página.
            if (data.html) {
                await window.SpaUtils.loadContent(fullUrl, cleanPagePath, data);
            } else {
                // Se a resposta não tem HTML, mas é um JSON, chamamos o inicializador.
            ...
            // Se a resposta não for JSON, trata como um carregamento de página normal
...
        // Limpa eventos de scripts anteriores para evitar duplicação
        if (typeof window.clearPageEvents === 'function') {
            window.clearPageEvents();
        }

        // Chamadas de scripts e inicializadores
...
        }
        window.SpaUtils.setupSpaForms(); // <--- CHAMA A NOVA FUNÇÃO AQUI
...
    // Remove o listener antigo e ineficaz de formulário
    // document.body.removeEventListener('submit', handleOldFormSubmit);

    try {
        await window.SpaUtils.loadScript(`${window.URLADM}assets/js/anuncio.js`);
...
    window.SpaUtils.callPageInitializer(pageInitializers[cleanInitialPagePath], initialUrl);
    window.SpaUtils.setupSpaForms(); // <-- CHAMA A NOVA FUNÇÃO AQUI TAMBÉM
