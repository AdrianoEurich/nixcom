...
// Função setupFormValidation removida - usando a versão mais completa abaixo
...
function setupInputMasks() {
    ...
    if (typeof $ === 'undefined' || typeof Inputmask === 'undefined') {
        console.error('ERRO JS: setupInputMasks - jQuery ou Inputmask não estão disponíveis');
        console.log('DEBUG JS: jQuery disponível:', typeof $ !== 'undefined');
        console.log('DEBUG JS: Inputmask disponível:', typeof Inputmask !== 'undefined');
        return;
    }

    const phoneNumberInput = document.getElementById('phone_number');
    if (phoneNumberInput) {
        $(phoneNumberInput).inputmask({ "mask": "(99) 99999-9999" });
...
async function loadAndPopulateLocations(anuncioData) {
    ...
    if (!ufSelect || !citySelect || !neighborhoodInput) {
        console.warn('AVISO JS: Elementos de localização não encontrados:');
        console.warn('  - ufSelect (state_id):', !!ufSelect);
        console.warn('  - citySelect (city_id):', !!citySelect);
        console.warn('  - neighborhoodInput (neighborhood_id):', !!neighborhoodInput);
        return;
    }
    ...
    const initialNeighborhoodName = anuncioData?.neighborhood_id; // Agora é para o input de texto
...
function initializeFormFields(form, anuncioData, formMode, userPlanType) {
    ...
    const textAndNumberFields = [
        ...
        'gender', 'nationality', 'ethnicity', 'eye_color', 'neighborhood_id' // Adicionado neighborhood_id aqui
    ];
...
function applyPlanRestrictions(userPlanType) {
    ...
    [videoUploadBoxes, audioUploadBoxes].forEach(mediaTypeBoxes => {
        ...
        mediaTypeBoxes.forEach(box => {
            ...
            if (userPlanType === 'free') {
                // Bloquear completamente vídeos e áudios no plano FREE
                box.classList.add('locked');
                if (premiumLockOverlay) premiumLockOverlay.style.display = 'flex';
                if (inputElement) inputElement.disabled = true;
                // Adicionar cursor not-allowed para indicar que está bloqueado
                ...
                // Bloquear completamente vídeos e áudios no plano BASIC
                box.classList.add('locked');
                if (premiumLockOverlay) premiumLockOverlay.style.display = 'flex';
                if (inputElement) inputElement.disabled = true;
                // Adicionar cursor not-allowed para indicar que está bloqueado
...
function setupAdminActionButtons(anuncioId, anuncianteUserId, currentAnuncioStatus) {
    ...
    if (btnActivate) {
        const canActivate = currentAnuncioStatus === 'pausado' || currentAnuncioStatus === 'pending';
