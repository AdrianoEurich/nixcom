<?php
// Endpoint com QR Code real usando API externa
...
error_log("PAYMENT_QR_API: Requisição recebida - " . date('Y-m-d H:i:s'));
...
try {
    ...
    if ($qrCodeData && strlen($qrCodeData) > 100) {
        ...
        echo json_encode([
            ...
            'payment_id' => 'qr_api_payment_' . time(),
        ...
        throw new \Exception('Falha ao gerar QR Code via API');
    }
} catch (\Exception $e) {
    error_log("PAYMENT_QR_API: Erro ao gerar QR Code - " . $e->getMessage());
